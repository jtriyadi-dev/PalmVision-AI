...
import { INVENTORY_REST_API_DOCS, INVENTORY_DB_SCHEMA_DOCS } from '../mockData';

export const InventoryApiDocsView: React.FC = () => {
  const [copiedCode, setCopiedCode] = useState(false);

  const handleCopy = () => {
    navigator.clipboard.writeText(INVENTORY_DB_SCHEMA_DOCS);
    setCopiedCode(true);
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs space-y-2">
        <h2 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
          ...
          Inventory, Warehouse & Procurement REST API & Database Schema DDL
        </h2>
        <p className="text-xs text-slate-500">
          Spesifikasi endpoint REST API dan struktur tabel relational database PostgreSQL
        </p>
      </div>
      ...
      <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs space-y-4">
        <h3 className="font-bold text-sm text-slate-900 dark:text-white flex items-center gap-2">
          <Terminal className="h-4 w-4 text-emerald-600" /> REST API Endpoints Specification (v1)
        </h3>
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-800">
          <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {INVENTORY_REST_API_DOCS.map((api, idx) => (
      ...
      <div className="p-5 rounded-2xl bg-slate-950 border border-slate-800 text-slate-200 space-y-3">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2 text-emerald-400 font-bold text-xs">
            <Database className="h-4 w-4" /> PostgreSQL DDL Migration Script (28 Tables)
          </div>
          <button
            onClick={handleCopy}
